...
for (let intervenant of personnes) {
  prof[intervenant] = allevent.filter((event) => {
    return event.title.includes(intervenant);
  });
}
// itération 3 fonction récupération des donnée formaté pour l'affichage sur le sunburst
...
am5.ready(function () {
  ...
  var yAxisheatmap = chart.yAxes.push(
    am5xy.CategoryAxis.new(root, {
      maxDeviation: 0,
      renderer: yRenderer,
      categoryField: "hour",
  ...
  var serieheatmap = chart.series.push(
    am5xy.ColumnSeries.new(root, {
  ...
  let handlerheatmap = function (ev) {
    ...
    joursemaine.forEach((jour) => {
      ...
      eventjour.forEach((event) => {
        for (let heure = 0; heure < 24; heure++) {
          ...
          if (!totalheure[jour][heurefinal]) {
            totalheure[jour][heurefinal] = {
              hour: heurefinal,
              weekday: joursfrancais[jour],
              value: 0,
            };
          }
      ...
      let heuredifzero = Object.values(totalheure[jour]).filter(
        (entry) => entry.value !== 0
      );
